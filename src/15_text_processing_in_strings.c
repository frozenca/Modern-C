...
int main() {
    const char* sample_text = "four people live for the fortune";

    assert(search(sample_text, "for"));
    assert(!search(sample_text, "form"));
    const char* new_sample_text = replace(sample_text, "fo", "of");
    printf("%s\n", new_sample_text);

    RegExpMatch result = (RegExpMatch) {0};
    regex_match(sample_text, "[a-f]", &result);
    print_regex_result(&result);

    const char* sample_text2 = "that that movie was bad";
    RegExpMatch result2 = (RegExpMatch) {0};
    regex_match(sample_text2, "(that) (\\1)", &result2);
    print_regex_result(&result2);

    const char* new_sample_text3 = regex_replace(sample_text, &result, "mario");
    printf("%s\n", new_sample_text3);
