...
int main() {
    const wchar_t* wsample_text = L"f\\u00dfur people live for the fortune";
    setlocale(LC_ALL, "en_US.utf8");
    char sample_text[256] = {0};
    const wchar_t* wquery = L"f\\u00dfr";
    char query_text[256] = {0};

    int return_value1 = wcstombs(sample_text, wsample_text, sizeof sample_text);
    int return_value2 = wcstombs(query_text, wquery, sizeof query_text);
    if (return_value1 > 0 && return_value2 > 0) {
        assert(search(sample_text, query_text));
    }

    const wchar_t* wreplace = L"\\U0001f34c";
    char replace_text[256] = {0};

    int return_value3 = wcstombs(replace_text, wreplace, sizeof replace_text);

    if (return_value3 > 0) {
        const char* new_sample_text = replace(sample_text, query_text, replace_text);
        wchar_t wnew_sample_text[256] = {0};
        mbstowcs(wnew_sample_text, new_sample_text, sizeof wnew_sample_text);
        wprintf(L"%s\n", wnew_sample_text);
    }
    ...
    const char* new_sample_text3 = regex_replace(sample_text, &result, replace_text);
    wchar_t wnew_sample_text3[256] = {0};
    mbstowcs(wnew_sample_text3, new_sample_text3, sizeof wnew_sample_text3);
    wprintf(L"%s\n", wnew_sample_text3);
